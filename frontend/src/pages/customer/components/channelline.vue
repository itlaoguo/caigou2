<template>
    <div id="ChannelLine" style="width: 100%;height: 100%;"></div>
</template>
...
import { getGender } from "@/api/customer/index"
...
const ongetExceedSpeedTrend = async () => {
    ...
    console.log('--------ongetExceedSpeedTrend---------', res);
    LineaData.value = res.data;
    // 实例不要使用ref 会导致第三方库二次渲染有问题  
    const myChart = echarts.init(document.getElementById("ChannelLine"));
    const option = {
        title: {
            text: ''
        },
        ...
        series: [
            {
                name: LineaData.value[0].gender.name,
                type: 'line',
                // stack: 'Total',
                symbol: 'circle',
                // 鼠标在图例上的效果
            ...
            {
                name: LineaData.value[1].gender.name,
                type: 'line',
                symbol: 'circle',
                // stack: 'Total',
                emphasis: {
            ...
            {
                name: LineaData.value[2].gender.name,
                type: 'line',
                symbol: 'circle',
                // stack: 'Total',
                emphasis: {
