...
const transform: AxiosTransform = {
  ...
  transformRequestHook:(res: any, options: any) => {
    ...
    if (res.data.code == 10005) {
      ...
      // console.log("========全局配置=====>", res.data);
      // console.log("========全局配置=====>",router);
    }
    ...
    if (res.data.code == 10001) {
      // 退出登录
      // console.log("========全局配置=====>", res.data.code);
      localStorage.clear();
    ...
    // 这里逻辑可以根据项目进行修改
    const hasSuccess = data && code == 10000;
    if (hasSuccess) {
      return data;
    }
  ...
  requestInterceptors: (config, options) => {
    ...
    if (token && (config as Recordable)?.requestOptions?.withToken !== false) {
      ...
      // 控制状态码
      (config as Recordable).headers['Request-from'] = 'dashboard';
