    ...
    <t-menu :class="menuCls" :theme="theme" :value="active" :collapsed="collapsed" :expanded="expanded"
      ...
      <template #logo>
        <span v-if="showLogo" :class="`${prefix}-side-nav-logo-wrapper`" @click="goHome">
          <img :src="getLogo()" style="margin-left: 8px;" alt="华能云成采购管理系统">
        </span>
      </template>
      <menu-content :nav-data="menu" />
      <template #operations>
        <span :class="versionCls"> {{ !collapsed ? '华能云成采购管理系统' : '' }} {{ pgk.version }} </span>
      </template>
    </t-menu>
...
// import logo from '@/assets/caigou-logo.png';
import AssetLogoFull from '@/assets/caigou-logo-full.png';
import AssetLogo from '@/assets/caigou-logo.png';
