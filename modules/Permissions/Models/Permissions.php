...
//        $model->setAttribute('permission_mark', $parent->permission_mark. '@'.  $model->permission_mark);
        $model->setAttribute('permission_mark', $$model->permission_mark);
...
//            $data['permission_mark'] = $parentMenu->permission_mark.'@'.$data['permission_mark'];
            $data['permission_mark'] = $data['permission_mark'];
        }
     ...
     * 获取权限树
     *
     * @param array $wherein 查询条件
     * @return array
     */
    public function getPermissionTree(array $wherein = [], int $parenetId = 0): array
    {
        $query = $this->newQuery()->where(['parent_id' => $parenetId])->where('type','!=', '3');
        ...
        // 获取所有权限
        $permissions = $query->orderBy('sort', 'asc')->get()->toArray();

        foreach ($permissions as &$permission) {
            // Recursive call to get child permissions
            $permission['children'] = $this->getPermissionTree($wherein, (int)$permission['id']);
        }
